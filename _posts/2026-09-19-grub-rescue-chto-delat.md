...
Сразу к делу: приглашение `grub rescue>` значит, что GRUB-загрузчик не нашёл свои файлы конфигурации. Система загружается через live-USB, монтируешь корневой раздел и ESP, делаешь `arch-chroot` и переустанавливаешь GRUB — после этого загрузка восстанавливается. Вся процедура занимает минут пятнадцать.
